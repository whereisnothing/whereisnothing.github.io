$(function() {
  ...
  var optionArea = $('<div/>').insertAfter($('#page-wrapper').find('.tpl')).addClass(attr('area')).slideUp();
  var loadingDom = $('<div />').appendTo(body);
  var message = $('<span />').appendTo($('#wrapper .navbar')).css({'padding':'15px','text-align':'center'});
  var tplBox=$('.tpl');
  ...
  for (var tpl in tplOptions.templates) {
    (function(tpl) {
      var td = $('.tpl .item a[href*="&tpl=' + tpl + '&"]').parents('ul.item');
      $('<span>设置</span>').insertBefore(td.find('li span.act')).addClass(attr('setting')).data('template', tpl);
    })(tpl);
  }
  /*
  //当前模板
  (function() {
	var current=$('.tpl .item li.title.active');
    var currentTemplate = current.parents('ul.item').find('img').attr('src').match(/\/templates\/(.*?)\/preview.jpg/)[1];
